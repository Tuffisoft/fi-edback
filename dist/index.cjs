...
function getFeedbackSchema() {
  return import_zod.z.object({
    ...
    // Pin color (hex code)
    pinColor: import_zod.z.string().regex(/^#[0-9A-Fa-f]{6}$/, "Must be a valid hex color code").optional(),
    // Viewport width for responsive positioning
    viewportWidth: import_zod.z.number().int().positive().optional(),
    // Device type for filtering
    deviceType: import_zod.z.enum(["mobile", "tablet", "desktop"]).optional()
  });
}
